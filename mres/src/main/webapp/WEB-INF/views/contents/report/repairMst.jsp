				...
				<c:if test = "${selectRepairList.repair_seq != 0}">
					...
					<tr>
						...
						<c:if test = "${selectRepairList.fr_img_name != ''}">
							<td class=><img src="${selectRepairList.fr_img_path}" width="300" height="210"></td> 
						</c:if>
						<c:if test = "${selectRepairList.fr_img_name == ''}">
							<td class="frUploadPopup"><label style="width:300px; height:210px;" >이미지 업로드</label></td>
						</c:if>
						<c:if test = "${selectRepairList.to_img_name != ''}">
						<td class=""><img src="${selectRepairList.to_img_path}" width="300" height="210"></td> 
						</c:if>
						<c:if test = "${selectRepairList.to_img_name == ''}">
							<td class="toUploadPopup"><label class="width:300px; height:210px;">이미지 업로드</label></td>
						</c:if>
					</tr> 
				</c:if>
			</c:forEach>
